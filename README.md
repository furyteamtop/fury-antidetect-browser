...
| Shared TOTP secrets | no. The schema has been ready since 0001 — `credentials.payload_enc` holds `{username, password, totp_secret, recovery_codes}` sealed client-side, and `autofill_tokens` hands one field to one person once — and not a line of it is written: no handlers, no agent support, no UI, no code generation. A feature that has not been started, said plainly rather than counted as partly done |
