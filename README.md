...
| Shared TOTP secrets | **local mode: done.** A profile has logins — username, password, two-factor seed — sealed with the machine key, and the code is generated in the agent so the seed never reaches the webview. All of RFC 6238's vectors pass, for SHA-1, SHA-256 and SHA-512. **Team mode: not yet** — the server's `credentials` table wants a blob sealed with the organisation key, and the shell says so rather than silently dropping what somebody typed |
