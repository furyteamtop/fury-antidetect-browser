...
Windows-конфигурация написана и ни разу не собиралась, и это не только вопрос
машины: серия у патча 0021 отдельно отмечает, что для Windows те же две подмены
надо ещё сделать в `scrollbar_theme_aura.cc` / `scrollbar_theme_fluent.cc`.
