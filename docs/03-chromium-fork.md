...
1. **Ломает наложение патчей.** Google правит те же файлы (`navigator.cc`, `webgl_rendering_context_base.cc`,
   ...
   `core/build/rebase.sh`. В серии сейчас 26 патчей, из них пять помечены суффиксом `!`
...
```
core/
├── patches/                            # 26 патчей на 02.08.2026
...
Что есть сегодня: `macos-arm64.gn`, `macos-arm64-lowmem.gn`, `macos-arm64-dev.gn` и
`windows-x64.gn`. Файла `macos-x64.gn` нет — `build.sh macos-x64` падает на проверке
наличия args-файла, — поэтому universal-бинарь через `lipo` пока план, а не шаг
процедуры; подсказку с готовой командой `build.sh` печатает, когда оба каталога
сборки действительно окажутся на месте. Windows-конфигурация написана и ни разу не
собиралась: единственный каталог в `core/src/out` — `macos-arm64-lowmem`, а серия у
патча 0021 отдельно отмечает, что для Windows те же две подмены надо ещё сделать в
`scrollbar_theme_aura.cc` / `scrollbar_theme_fluent.cc`.
