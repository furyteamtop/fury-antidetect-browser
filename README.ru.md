...
| | |
|---|---|
| Сборка под Windows и Linux | патчи написаны под них и ни разу там не собирались |
...
| QUIC / HTTP-3 | выключен, по той же причине. Настоящий Chrome поднимает HTTP/3 там, где его предлагают, а Fury — никогда, и это видит любой сервер, анонсирующий `Alt-Svc` |
| Скрыть CDP от замера времени | нет. С присоединённым отладчиком `console.debug` большого объекта идёт примерно в тринадцать раз дольше — измерено и в настоящем Chrome, так что это детект автоматизации, а не Fury; но если вы ведёте профиль скриптом, скрывается именно она |
...
| Row-level security на сервере | объявлена в схеме и не работает: `bind_rls_user` не вызывается ниоткуда, поэтому `app.user_id` в соединении не выставляется. RBAC на хендлерах при этом есть и работает |
