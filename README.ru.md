...
> агент на нём запускается, а `tools/verify-windows.ps1` проходит 29 утверждений
...
> стороны оператора.
>
> **Пока ничего не подписано, и решается это бумагами, а не кодом.** Идёт
> получение статуса разработчика Apple: сертификат оплачен, аккаунт проходит
> проверку Apple — именно её ждут Developer ID и нотаризация. Инструменты уже
> написаны и лежат без дела:
> [tools/release/sign-core.sh](tools/release/sign-core.sh),
> [sign-shell.sh](tools/release/sign-shell.sh), [docs/17](docs/17-apple-signing.md).
>
> Пока этого нет: скачанная сборка для macOS подписана ad-hoc, поэтому Gatekeeper
> отказывается её открывать со словами «повреждена» — это про отсутствующую
> подпись, а не про испорченный файл, — а Windows показывает предупреждение
> SmartScreen. Сборка из исходников обходит и то, и другое. До сертификата любой
> релиз — предварительный.
>
