...
> помечены как предварительные и **пока не подписаны** — об этом ниже.
>
...
> стороны оператора.
>
> **Подписанного релиза пока нет, но бумажная половина позади.** Статус
> разработчика Apple, оплаченный 15.08.2026, пришёл 21.09.2026: сертификат
> Developer ID Application есть, и [sign-shell.sh](tools/release/sign-shell.sh)
> в тот же день впервые отработал на настоящей подписи — и первым же прогоном
> нашёл дефект в самом скрипте: `pipefail` объявлял hardened runtime
> отсутствующим у бандла, где он был. Релиз 0.1.4 вышел тем же утром, до
> сертификата, и подписан ad-hoc, как и предыдущие. Осталась не очередь, а
> релиз: [sign-core.sh](tools/release/sign-core.sh), sign-shell.sh,
> нотаризация, [docs/17](docs/17-apple-signing.md). Windows — отдельный
> сертификат, и он не начат.
>
> Пока такого релиза нет, скачанные сборки всё равно запускаются — нужен один
> лишний шаг. Сборка для macOS подписана ad-hoc, поэтому Gatekeeper отказывается её
> открывать со словами «повреждена» — это про отсутствующий Developer ID, а не
> про испорченный файл. Перетащите Fury в «Программы» и один раз выполните:
>
> ```bash
> xattr -dr com.apple.quarantine /Applications/Fury.app
> ```
>
> или без терминала: попробуйте открыть, затем Системные настройки →
> Конфиденциальность и безопасность → **Всё равно открыть**. Windows показывает
> предупреждение SmartScreen: **Подробнее → Выполнить в любом случае**. Сборка
> из исходников обходит и то, и другое. В любом случае до подписанного релиза
> любой релиз — предварительный; по шагам — в [docs/15](docs/15-install.ru.md).
>
...
- **core** — форк Chromium 153, [27 патчей](core/patches/); подмена в C++, не в JS
...
| Подпись и нотаризация | сертификат Developer ID пришёл 21.09.2026, инструменты написаны ([sign-core.sh](tools/release/sign-core.sh), [sign-shell.sh](tools/release/sign-shell.sh)); релиза, собранного с ним, ещё нет — 0.1.4 вышел раньше. До такого релиза macOS говорит «повреждён» (снять карантинный флаг или «Конфиденциальность и безопасность → Всё равно открыть» — [docs/15](docs/15-install.ru.md)). Windows нужен свой сертификат, не начат, и там SmartScreen |
