...
| Общие TOTP-секреты | нет. Схема готова с 0001 — `credentials.payload_enc` хранит `{username, password, totp_secret, recovery_codes}`, запечатанные на клиенте, а `autofill_tokens` выдаёт одно поле одному человеку один раз — и не написано ни строки: ни хендлеров, ни поддержки в агенте, ни интерфейса, ни генерации кодов. Не начатая функция, сказанная прямо, а не засчитанная наполовину |
