...
>
> **Ничего не подписано.** macOS откажется открывать скачанное, Windows покажет
> предупреждение; сертификат Apple оплачен и ждёт активации
> ([docs/17](docs/17-apple-signing.md)). До этого релиз — предварительный.
>
