...
| Общие TOTP-секреты | **локально: сделано.** У профиля есть логины — имя, пароль, секрет второго фактора — запечатанные ключом машины, а код считается в агенте, поэтому секрет не попадает в webview. Все векторы RFC 6238 сходятся, для SHA-1, SHA-256 и SHA-512. **В команде: пока нет** — серверная таблица `credentials` ждёт блоб, запечатанный ключом организации, и оболочка говорит об этом прямо, а не молча теряет введённое |
