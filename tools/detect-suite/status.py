...
def build():
    ...
    # The commit that last touched the CAPTURES, not HEAD.
    #
    # HEAD was the obvious choice and it is wrong: the page would go stale on
    # every unrelated commit, so `--check` failed in CI the first time anything
    # else was committed. This says when the measurement was taken, which is
    # the question a reader actually has, and only changes when a measurement
    # does.
    try:
        commit = subprocess.run(
            ["git", "log", "-1", "--format=%h %ad", "--date=short", "--",
             "baselines/ctx-fury-redacted.json",
             "baselines/ctx-chrome-redacted.json",
             "baselines/gate-persona-redacted.json"],
...
<footer>
  <p>Generated by <code>tools/detect-suite/status.py</code> from the redacted
     captures tracked in the repository{f", last measured in <code>{e(commit)}</code>" if commit else ""}.
